...
|---------|------|------------|
| _unreleased_ | 2026-04-07 | **Harness web integrations** (CLI-only, no dylib bump) - 7 new commands: `mod changelog/versions/updated` (Nexus `/changelogs.json`, `/files.json`, `/updated.json`), `wiki spell/item/verify/clear-cache` (bg3.wiki MediaWiki via opensearch + parse — no Cargo). Tolerant `_version_sort_key` (SemVer + month-name dates), path-aware 403 classifier (`_classify_403` with `/mods/\d+` regex), SHA-1-keyed wiki cache with alias-pointer files and path-containment sandbox, 37 new offline tests (20 nexus + 17 wiki). Plan: `~/.claude/plans/2026-04-06-bg3se-harness-opencli-integration.md` Chunks 1–2 |
