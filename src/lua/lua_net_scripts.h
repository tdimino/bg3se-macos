...
"}\n"
"\n"
"-- CreateChannel: builds a NetChannel whose Send/Request/Broadcast route through\n"
"-- Ext.Net.PostMessage* -> the in-process message bus (message_bus.c). It never\n"
"-- touches the game's net::MessageFactory / ProtocolList / SendToPeer, so it is\n"
"-- crash-safe in single-player (client+server share one process/Lua state).\n"
"local _CreateChannel = function(module, channel)\n"
...
"        if k == 'CreateChannel' then return _CreateChannel end\n"
"        return Ext.Net[k]\n"
"    end\n"
"})\n"
"\n"
"-- Expose Ext.Net.CreateChannel (MCM / newer mods look for it here). Safe now\n"
"-- that NetChannels are backed by the local message bus rather than the game's\n"
"-- net-message factory. The previously-crashing path (ExtenderProtocol insertion\n"
"-- + SendToPeer with the out-of-range id-400 message) is opt-in only, behind the\n"
"-- BG3SE_NET_RAKNET env flag in net_hooks.c, and is NOT reachable from here.\n"
"Ext.Net.CreateChannel = _CreateChannel\n"
"\n"
