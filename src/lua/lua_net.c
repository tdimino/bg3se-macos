...
static bool s_is_server_context = false;
...
// ============================================================================
// Shared Helpers (eliminates duplication across PostMessage variants)
// ============================================================================

/**
 * If the Lua value at `handler_idx` is a function, register it as a callback.
 * Returns the assigned request_id (0 if no callback).
 * On registration failure, raises a Lua error (does not return).
 */
static uint64_t maybe_register_callback(lua_State *L, int handler_idx) {
    if (!lua_isfunction(L, handler_idx)) {
        return 0;
    }

    lua_pushvalue(L, handler_idx);
    uint64_t request_id = callback_registry_register(L);
    if (request_id == 0) {
...
/**
 * Queue a message, cleaning up the callback on failure.
 * On queue failure, raises a Lua error (does not return).
 */
static void queue_or_error(lua_State *L, NetMessage *msg, uint64_t request_id) {
    if (!message_bus_queue(msg)) {
        if (request_id != 0) {
            callback_registry_cancel(L, request_id);
        }
        message_free(msg);
        luaL_error(L, "Failed to queue message");
        // luaL_error does not return
    }
    message_free(msg);
}
...
static int lua_net_post_to_user(lua_State *L) {
    ...
    uint64_t request_id = maybe_register_callback(L, 5);
...
static int lua_net_is_host(lua_State *L) {
    lua_pushboolean(L, message_bus_is_host(s_is_server_context));
    return 1;
}
...
static bool s_scripts_loaded = false;
